...
class ReportCommandUseCaseImplTest {

    @InjectMocks // 테스트 대상 클래스. @Mock으로 생성된 가짜 객체들이 여기에 주입됩니다.
    private ReportCommandUseCaseImpl reportJpaUseCase;

    @Mock
    private ReportCommandAdapter reportCommandAdapter;
    ...
    void saveReport_Success_WithAllKeywords() {
        // given
        ReportCommandUseCase.ReportCreateDto dto = new ReportCommandUseCase.ReportCreateDto(
                1L,
                101L,
                ReportType.SAFE,
                "테스트 내용입니다.",
        ...
        when(reportCommandAdapter.saveReport(any(ReportCommandUseCase.ReportCreateDto.class))).thenReturn(expectedReportId);

        // when
        reportJpaUseCase.saveReport(dto);

        // then
        verify(reportCommandAdapter, times(1)).saveReport(dto);
